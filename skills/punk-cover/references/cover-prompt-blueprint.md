...
Create one {platform} editorial cover at {ratio}{output_dimensions_if_any}. Organize every visual decision through the concrete direction below, not as a decorative filter.
...
Compose specifically for {ratio}; keep the topic legible in a fast social feed. Fuse text and image instead of placing a caption over an unrelated illustration. The visual direction determines where text lives and how it interacts with objects, textures, geometry, and depth.

## Typography

When text is requested, keep the complete title accurate and readable; do not misspell, crop, or excessively distort it. Use little supporting text, with correct Chinese characters where applicable. Only when the user or visual direction explicitly requests no text, use title fields solely to derive the visual and render no typography. Supporting text must clarify the concept, never become filler.
